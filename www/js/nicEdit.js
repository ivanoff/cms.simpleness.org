...
var nicEditorPanel = bkClass.extend({
	construct : function(e,options,nicEditor) {
		...
		this.panelElm = new bkElement('DIV').setStyle({width:'901px', margin : '2px auto', marginTop : '0px', zoom : 1, overflow : 'hidden'}).addClass('panel').appendTo(this.panelContain);
...
var nicEditorSelect = bkClass.extend({
	...
	construct : function(e,buttonName,options,nicEditor) {
		...
		this.txt = new bkElement('div').setStyle({overflow : 'hidden', 'float' : 'left', width : '66px', height : '18px', marginTop : '0', fontFamily : 'sans-serif', textAlign : 'center', fontSize : '10px'}).addClass('selectTxt').appendTo(this.items);
...
var nicSEOOptions = {
	buttons : {
		'SEO' : { name : 'Edit headers', type : 'nicSEOButton'}
	}
	, iconFiles : { 'SEO' : '/images/btn_seo.gif' }
};
/* END CONFIG */

var nicSEOButton = nicEditorAdvancedButton.extend({
	width : '550px',
...
nicEditors.registerPlugin(nicPlugin,nicSEOOptions);
...
nicEditors.registerPlugin(nicPlugin,nicSpOptions);



/* START CONFIG */
var nicLogoutOptions = {
	buttons : {
		'logout' : { name : 'Logout', type : 'nicLogoutButton'}
	}
	, iconFiles : { 'logout' : '/images/btn_logout.gif' }
};
/* END CONFIG */

var nicLogoutButton = nicEditorButton.extend({
	mouseClick : function() {
//	    if (confirm("Are you sure you want to exit?")) {
		window.location.replace("/login/exit");
//	    }
	}
});

nicEditors.registerPlugin(nicPlugin,nicLogoutOptions);
